// start time setting
const start_hours = 16, minutes = 30, seconds = 59
const end_hours = 17, end_minutes = 59, end_seconds = 59
...
const openBrowser = async (index) => {
    ...
    ppts[index].use(
        RecaptchaPlugin({
            provider: {
                id: '2captcha',
                token: '317c51c69b8919942581b0548fd142b7'
            },
            visualFeedback: true
        })
    )
...
const goToStep3 = async (index, carIndex, dataObject) => {
    ...
    while (true) {
        ...
        const { captchas, solutions, solved, error } = await pages[index].solveRecaptchas();
        console.log("reCaptcha solved.");
        
        await pages[index].evaluate(() => {
            document.querySelector('#registrationMark').value = ''
        })
        await pages[index].type('#registrationMark', data[0])
        await pages[index].evaluate(() => {
            document.querySelector('#registrationMarkAgain').value = ''
        })
        await pages[index].type('#registrationMarkAgain', data[0])
        await Promise.all([
...
const onTypeAddressBlock = async (index, addressBlock) => {
    while (true) {
        await delay(500);
        try {
            await pages[index].type('#addressBlock', addressBlock)
            break
        } catch (err) {
...
const onSelectAddressArea = async (index, addressArea) => {
    try {
        const alias_area = areas[addressArea]
        await pages[index].select("#addressArea", alias_area);
    } catch (error) {
...
const onSelectAddressDistrict = async (index, addressArea, addressDistrict) => {
    try {
        const areaDistricts = districts[addressArea];
        const selected = areaDistricts.filter(d => d.short === addressDistrict);
        await pages[index].select('#addressDistrict', selected[0].short);
    } catch (error) {
        console.log('Error happened when addressDistrict enter. You can try on hand!' + error + 'alias: ' + addressArea)
    }
}
